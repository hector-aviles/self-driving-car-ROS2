#!/usr/bin/env python3
"""
This node implements the action policy using Logistic Regression
"""
import rclpy
...
class TestLRNode(Node):
    def __init__(self, speed_left, speed_right):
        super().__init__('test_LR')
    ...
    # Callback methods (same as test_CART.py)
    ...
    # Behavior methods (same as test_CART.py)
    def cruise(self):
...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: ros2 run self_driving_car test_LR.py speed_left speed_right (m/s)")
        return
    
    speed_left = sys.argv[1]
    speed_right = sys.argv[2]
    
    node = TestLRNode(speed_left, speed_right)
