...
class BMWX5Controller(Node):
    ...
    def create_messages(self):
        ...
        self.msg_point_cloud.width = self.lidar.getNumberOfPoints()
        self.msg_point_cloud.point_step = 20
        self.msg_point_cloud.row_step = 20 * self.lidar.getNumberOfPoints()
    ...
    def publish_sensor_data(self, current_t):
        ...
        if (current_t - self.time_lidar_last) >= self.lidar_interval:
            ...
            self.msg_point_cloud.data = self.lidar.getPointCloud(data_type='buffer')
            self.msg_point_cloud.header.stamp = self.get_clock().now().to_msg()
            self.pub_point_cloud.publish(self.msg_point_cloud)
            self.get_logger().debug('Published LiDAR point cloud')
        ...
        if (current_t - self.time_imu_last) >= self.imu_interval:
            ...
            self.msg_accel.linear_acceleration.x = accel_values[0]
            self.msg_accel.linear_acceleration.y = accel_values[1]
            self.msg_accel.linear_acceleration.z = accel_values[2]
