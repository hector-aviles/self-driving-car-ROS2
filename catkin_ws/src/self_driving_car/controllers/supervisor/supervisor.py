...
class SupervisorNode(Node):
    def __init__(self):
        ...
        self.publishers = {}
        for i in range(1, 11):
            self.publishers[f'car_{i}'] = self.create_publisher(Pose2D, f'/car_{i}_pose', 10)
    ...
    def initialize_vehicle_positions(self):
        ...
        for i, car in enumerate(self.cars):
            if car is not None:
                ...
                self.get_logger().debug(f'Car {i+1} initial position: ({x}, {y})')
                ...
                self.get_logger().debug(f'Car {i+1} new position: ({values[0]}, {values[1]})')
            else:
                self.tf.append(None)
                self.get_logger().warning(f'Car {i+1} not found in simulation')
    
    def callback_speed_cars_left_lane(self, msg):
        """Callback for left lane speed"""
        self.speed_cars_left_lane = msg.data
        self.get_logger().debug(f'Left lane speed set to: {msg.data}')
    
    def callback_speed_cars_right_lane(self, msg):
        """Callback for right lane speed"""
        self.speed_cars_right_lane = msg.data
        self.get_logger().debug(f'Right lane speed set to: {msg.data}')
    ...
    def control_loop(self):
        ...
        # Only start processing after start signal is received
        if not self.start_signal_received:
            return
        ...
        for i, car in enumerate(self.cars):
            if car is not None and self.tf[i] is not None:
                ...
                msg_car_pose.theta = 0.0  # 2D pose, z rotation not used
                ...
                if publisher_name in self.publishers:
                    self.publishers[publisher_name].publish(msg_car_pose)
        
        # Publish BMW pose
        self.publish_bmw_pose()
    
    def publish_bmw_pose(self):
        """Publish BMW X5 position and orientation"""
        if self.bmw is not None:
            ...
            if self.log_counter % 100 == 0:  # Log every 100 steps
                self.get_logger().debug(f'BMW Position: ({msg_bmw_pose.x:.2f}, {msg_bmw_pose.y:.2f})')
