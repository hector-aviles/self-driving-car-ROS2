...
class TestCARTNode(Node):
    def __init__(self, speed_left, speed_right):
        super().__init__('test_CART')
    ...
    def main_loop(self):
        ...
        if self.action == "cruise":
            ...
            # Implement proper waiting mechanism for ROS2
...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: ros2 run self_driving_car test_CART.py speed_left speed_right (m/s)")
        return
    
    speed_left = sys.argv[1]
    speed_right = sys.argv[2]
    
    node = TestCARTNode(speed_left, speed_right)
