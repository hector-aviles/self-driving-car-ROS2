#!/usr/bin/env python3
"""
This node implements the action policy using XGBoost
"""
import rclpy
from rclpy.node import Node
from std_msgs.msg import Empty, Bool, String, Float64
import sys
import pandas as pd
import pickle
import time
...
class TestXGBoostNode(Node):
    def __init__(self, speed_left, speed_right):
        super().__init__('test_XGBoost')
        ...
        self.get_logger().info(f"Loading model from: {file}")
        
        try:
            self.model = pickle.load(open(file, 'rb'))  
        except Exception as error:
            self.get_logger().error(f"Error loading model: {error}")

        # Create and configure the label encoder for decoding
        self.encoder = LabelEncoder()
        self.encoder.classes_ = np.array(['change_to_left', 'change_to_right', 'cruise', 'keep'])
    ...
    # Callback methods (same as other policy nodes)
    ...
    # Behavior methods (same as other policy nodes)
    def cruise(self):
    ...
    def main_loop(self):
        ...
        try:
            ...
            y_numeric = self.model.predict(X)
            action_encoded = y_numeric[0]
            self.action = self.encoder.inverse_transform([action_encoded])[0]
            end_time = time.time()
        ...
        self.get_logger().info(f"Action (decoded): {self.action}")
...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: ros2 run self_driving_car test_XGBoost.py speed_left speed_right (m/s)")
        return
    
    speed_left = sys.argv[1]
    speed_right = sys.argv[2]
    
    node = TestXGBoostNode(speed_left, speed_right)
