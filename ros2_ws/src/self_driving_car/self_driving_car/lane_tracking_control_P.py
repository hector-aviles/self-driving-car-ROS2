...
are given by two straight lines in rho-theta form.
"""
import rclpy
...
class LaneTrackingNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("INITIALIZING LANE TRACKING NODE...")
        self.get_logger().info(
            f"Using max_speed={self.max_speed}, "
            f"k_rho={self.k_rho}, k_theta={self.k_theta}"
        )
    ...
    # Control law
    # ------------------------------------------------
    def calculate_control(self, rho_l, theta_l, rho_r, theta_r, dist=None):
        error_rho_l = self.goal_rho_l - rho_l
        error_theta_l = self.goal_theta_l - theta_l
        error_rho_r = rho_r - self.goal_rho_r
        error_theta_r = theta_r - self.goal_theta_r

        if rho_l != 0 and rho_r != 0:
            error_rho = (error_rho_l + error_rho_r) / 2.0
            error_theta = (error_theta_l + error_theta_r) / 2.0
        elif rho_l != 0:
    ...
    # ------------------------------------------------
    # Update (called explicitly from main loop)
    # ------------------------------------------------
    def update(self):
        if not (self.enable_cruise or self.enable_follow):
            return
        ...
        speed, steering = self.calculate_control(
            ...
            self.lane_rho_r,
            self.lane_theta_r,
            self.dist_to_obstacle if self.enable_follow else None
        )
