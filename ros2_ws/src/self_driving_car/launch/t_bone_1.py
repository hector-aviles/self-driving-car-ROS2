...
def find_world_file(package_dir):
    """Locate t_bone_1.wbt in typical project locations."""
    search_paths = [
        os.path.join(package_dir, "worlds", "t_bone_1.wbt"),
        os.path.join(os.path.dirname(package_dir), "..", "src",
                     "self_driving_car", "worlds", "t_bone_1.wbt"),
...
def generate_launch_description():
    ...
    if not world_file:
        return LaunchDescription([
            LogInfo(msg="[t_bone_1.py] ERROR: t_bone_1.wbt not found!")
        ])
    ...
    log_env = LogInfo(
        msg=f"[t_bone_1.py] WEBOTS_PROJECT_PATH = {project_dir}"
    )
