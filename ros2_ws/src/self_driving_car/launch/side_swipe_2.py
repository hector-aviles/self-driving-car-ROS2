...
def find_world_file(package_dir):
    """Locate side_swipe_2.wbt in typical project locations."""
    search_paths = [
        os.path.join(package_dir, "worlds", "side_swipe_2.wbt"),
        os.path.join(os.path.dirname(package_dir), "..", "src",
                     "self_driving_car", "worlds", "side_swipe_2.wbt"),
...
def generate_launch_description():
    ...
    if not world_file:
        return LaunchDescription([
            LogInfo(msg="[side_swipe_2.py] ERROR: side_swipe_2.wbt not found!")
        ])
    ...
    log_env = LogInfo(
        msg=f"[side_swipe_2.py] WEBOTS_PROJECT_PATH = {project_dir}"
    )
