...
def find_world_file(package_dir):
    ...
    search_paths = [
        ...
        os.path.join(os.path.dirname(package_dir), "..", "src",
                     "get_lane_params", "worlds", "reference.wbt"),
...
def find_project_path(package_dir):
    ...
    src_dir = os.path.join(
        os.path.dirname(package_dir), "..", "src", "get_lane_params"
    )
...
def generate_launch_description():
    ...
    pkg_dir = get_package_share_directory("get_lane_params")
    world_file = find_world_file(pkg_dir)

    if not world_file:
        return LaunchDescription([
            LogInfo(msg="[reference.py] ERROR: reference.wbt not found!")
        ])
    ...
    webots_process = ExecuteProcess(
        cmd=[
            "webots",
            "--mode=realtime", # Options: 'realtime', 'fast', 'pause'
            "--stdout",
            "--stderr",
            world_file
        ],
    ...
    ros_nodes = [  
        ...
        Node(package="get_lane_params", executable="lane_detector_canny_hough",
